...
import CanvasCards from '../components/CanvasCardsLsa';

import * as cardStyles from '../styles/cardsLsaTemplate.module.css';

function CardStandardTemplate( {data} ) {

    //console.log(data);

    const {color, contacts, email, instagram, linkedin, job, maps, name, phone, photo, address, slug, surname, website, whatsapp} = data.info.nodes[0].frontmatter;
    ...
    return ( 
        <div>
            <div className={cardStyles.infoCont}>
                <div className={cardStyles.interactiveAssets}>
                    ...
                    <div className={cardStyles.iconsCont}>
                        <a href={instagram} target="_blank" rel="noreferrer" aria-label="instagram"><div className={`${cardStyles.iconBC} ${cardStyles.instagramBC}`} placeholder="instagram"></div></a>
                        <a href={linkedin} target="_blank" rel="noreferrer" aria-label="linkedin"><div className={`${cardStyles.iconBC} ${cardStyles.linkedinBC}`} placeholder="linkedin"></div></a>
                        ...
                        <a href={website} target="_blank" rel="noreferrer" aria-label="web"><div className={`${cardStyles.iconBC} ${cardStyles.webBC}`} placeholder="web"></div></a>
            ...
            <CanvasCards color={color} logo={'/lsa_logo3D.glb'}/>
        </div>
     );
}
